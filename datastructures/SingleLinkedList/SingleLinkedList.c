...
_SlinkedList_state InitializeSLinkedList(SlinkedList *x, int elementSize, FreeFunction freeFN,
...
_SlinkedList_state DisposeSLinkedList(SlinkedList *x) {
...
_SlinkedList_state SLinkedListAddFront(SlinkedList *x, void *elementAddress) {
...
_SlinkedList_state SLinkedListRemoveFront(SlinkedList *x) {
...
_SlinkedList_state SLinkedListRemoveFrontOnly(SlinkedList *x) {
    //if there is no data just return
    if (SLinkedListSize(x) == 0) return _SlinkedList_SIZEERR;
    //fix the end pointer
    if (SLinkedListSize(x) == 1)
...
_SlinkedList_state SLinkedListPeekFront(SlinkedList *x, void *targetAddress) {
    //if there is no data just return
    if (SLinkedListSize(x) == 0) return _SlinkedList_SIZEERR;
...
_SlinkedList_state SLinkedListExtractFront(SlinkedList *x, void *targetAddress) {
    //write the data then remove it
    SLinkedListPeekFront(x, targetAddress);
    return SLinkedListRemoveFront(x);
}

_SlinkedList_state SLinkedListAddBack(SlinkedList *x, void *elementAddress) {
...
_SlinkedList_state SLinkedListRemoveBack(SlinkedList *x) {
...
_SlinkedList_state SLinkedListRemoveBackOnly(SlinkedList *x) {
    //if there is no nodes just return
    if (SLinkedListSize(x) == 0) return _SlinkedList_SIZEERR;
    //else get to the wanted node
    Snode *curr = x->head;
...
_SlinkedList_state SLinkedListPeekBack(SlinkedList *x, void *targetAddress) {
    //if there is no data just return
    if (SLinkedListSize(x) == 0) return _SlinkedList_SIZEERR;
...
_SlinkedList_state SLinkedListExtractBack(SlinkedList *x, void *targetAddress) {
    //write the data then rmeove the data
    SLinkedListPeekBack(x, targetAddress);
    return SLinkedListRemoveBack(x);
}

_SlinkedList_state SLinkedListGet(SlinkedList *x, int index, void *targetAddress) {
...
_SlinkedList_state SLinkedListRemove(SlinkedList *x, int index) {
    //if the index bigger than or equal the list, return
    if (index >= SLinkedListSize(x) || index < 0) return _SlinkedList_SIZEERR;
...
_SlinkedList_state SLinkedListInsert(SlinkedList *x, int index, void *elementAddress) {
...
_SlinkedList_state SLinkedListSearch(SlinkedList *x, void *keyAddress, CompareFunction cmpFN, int *index) {
...
void SLinkedListGetIterator(SlinkedList *x, SIterator *y) {
    y->node = x->head;
    y->list = x;
}
...
char *stateToMsg(_SlinkedList_state state) {
