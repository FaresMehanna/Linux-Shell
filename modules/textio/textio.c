...
static inline _text_io_state addStrToLinkedList(struct string *currWord, SlinkedList *parameters) {
    char *toSend;
    if (trimToActSize(currWord) != _text_io_SUCC) {
        return _text_io_MEMERR; //remove any un-needed space
    }

    getCharPointer(currWord, &toSend);  //get pointer to char*
    if (strlen(toSend) != 0) {
        if (SLinkedListAddBack(parameters, &toSend) != _SlinkedList_SUCC) {
            return _text_io_MEMERR; //add to parameter list
        }
    }

    if (createString(currWord) != _text_io_SUCC) {
        return _text_io_MEMERR; //create new string
    }
...
_text_io_state getParameters(char *line, SlinkedList *parameters) {
    ...
    while (line[curr] != '\0') {
        ...
        switch (line[curr]) {
            ...
            case ' ':
                ...
                if (inWord && inDubCout) {
                    if (addCharToString(&currWord, line[curr]) != _text_io_SUCC) {
                        return _text_io_MEMERR;
                    }
                ...
                if (addStrToLinkedList(&currWord, parameters) != _text_io_SUCC) {
                    return _text_io_MEMERR;
                }
    ...
    //last word
    if (inWord && addStrToLinkedList(&currWord, parameters) != _text_io_SUCC) {
        return _text_io_MEMERR;
    }
