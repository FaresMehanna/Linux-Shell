...
static inline _text_io_state createString(struct string* str){
	...
	if(!str->data)	return _text_io_MEMERR;
	str->data[0] = '\0';
	str->actLength = 1;
	str->memLength = 4;
...
static inline _text_io_state growString(struct string* str){
	if(str->actLength != str->memLength)	return _text_io_SUCC;
	char* new = srealloc(str->data,str->memLength*2);
	if(!new)	return _text_io_MEMERR;
...
static inline _text_io_state addCharToString(struct string* str, char ch){
	if(growString(str) != _text_io_SUCC)	return _text_io_MEMERR;
