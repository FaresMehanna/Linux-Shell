...
static inline _batch_mode_state startBatchMode(char* filePath){
	//try to open the batch file
	struct fileDate file;
	if(setFile(&file,filePath) != _file_io_SUCC){
		printToUser("BATCH MODE MODULE","FILE NOT FOUND",_shell_io_msg_state_ERROR,0,0);				
	...
	while(s == _file_io_SUCC || s == _file_io_DONE){
		...
		if(read(0,buff,1) == 0){memoryClear();	return _batch_mode_SUCC;}
		//print the curr Line
		printf(">> %s\n",currLine);
		...
		if(getParameters(currLine,&parameters) != _text_io_SUCC){
			printToUser("BATCH MODE MODULE","MEMORY ERROR",_shell_io_msg_state_WARNING,0,0);
			closeFile(&file);				
		...
		if(s == _file_io_DONE)	break;
		s = readFile(&file,&currLine);	//read again
	}

	//if there is error in reading a line
	if(s == _file_io_FILE_INPUTERR){
		printToUser("BATCH MODE MODULE","LENGTH EXCEEDED",_shell_io_msg_state_WARNING,0,0);
		closeFile(&file);
		return _batch_mode_FILEERR;
	}
