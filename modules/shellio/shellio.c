_shell_io_state getInputFromUser(SlinkedList *parameters) {
    ...
    if (s == _text_io_SUCC_DONE && feof(stdin)) {
        return _shell_io_END;
    }

    if (s == _text_io_ERR || s == _text_io_MEMERR) {
        return _shell_io_MEMERR;
    }
    ...
    //get the parameters
    if (getParameters(line, parameters) != _text_io_SUCC) {
        return _shell_io_MEMERR;
    }
...
_shell_io_state printToUser(char *identifier, char *message, shell_io_msg_state state) {
...
int handleVar(char *oStr, struct string *str, int index, int len) {
        ...
        switch (str[i]) {
            ...
            case '~':
                ...
                if ((i - 1 >= 0 && str[i - 1] == ' ' && i + 1 == len) ||
                    ...
                    lenty = 0;
                    while (homeTemp[lenty] != '\0') {
                        addCharToString(&toReturn, homeTemp[lenty++]);
                    }
