...
static inline void executeShell(char *path, char **params, int background) {
    ...
    if (pid == 0) {
        ...
        execv(path, params);
        printToUser("CHILD PROCESS", "CAN'T FORK", _shell_io_msg_state_ERROR);
        exit(EXIT_FAILURE);
        ...
        if (writeToLog(buff) == _log_io_FILE_ERR) {
            printToUser("EXECV EXECUTOR", "LOG FILE ISN'T ACCESSIBLE", _shell_io_msg_state_WARNING);
        }
        ...
        if (!background) {
            ...
            printToUser("EXECV EXECUTOR", "CHILD PROCESS STARETED IN BACKGROUND", _shell_io_msg_state_INFO);
        }

    } else {
        printToUser("PARENT PROCESS", "CAN'T FORK", _shell_io_msg_state_ERROR);
    }
}
